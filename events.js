function renderEventsClass() {
    ...
    events.forEach(event => {
      ...
      const main = document.getElementById('formDisplay');
      ...
      buyTicketsButton.addEventListener('click', () => {
        ...
        newForm.classList.add('popup');
        //code for form
        newForm.innerHTML =`
        <div class="overlay" id="paymentFormDisplay">
        <div class="content">
        <div class="modal">
 ...
 <div class="payment--options">
  ...
  <div class="credit-card-info--form">
    <div class="input_container">
       <label for="password_field" class="input_label">Name and Surname</label>
       <input id="password_field" class="input_field" type="text" name="input-name" title="Input full name" placeholder="Enter your full name" required>
    </div>
    <div class="input_container">
       <label for="password_field" class="input_label">Card Number</label>
       <input id="password_field" class="input_field" type="number" name="input-name" title="Input card No." placeholder="0000 0000 0000 0000" required>
    </div>
    <div class="input_container">
    ...
    <button class="purchase--btn" id="purchaseButton">Purchase</button>
</form>
</div>
</div>
</div><!-- end of div class modal -->
        `;
        location.href = '#formDisplay';
        main.appendChild(newForm);
        const payment = 
        document.getElementById("purchaseButton");
        payment.addEventListener('click', function(form){
          form.preventDefault();
          const input = 
          document.getElementById("password_field");
          let paymentFormDisplay = document.getElementById("paymentFormDisplay")

          const forAllInputs = input.value.trim();
          if(forAllInputs ==='') {
            alert(`error encountered while booking a ticket for a ${event.location} ${event.name} event for the price of $${event.price}`);
          }else{
            alert(`Payment of $${event.price} was made successfuly, for an ${event.category} by ${event.location}'s ${event.name} event at ${event.time} for ${event.date}`);
          }
          //redirects back to ticket page
          paymentFormDisplay.remove();
        });
      });
    });
  }
